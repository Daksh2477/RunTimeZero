...
// @ts-expect-error — generated by `npm run physics:build`
...
simulateRouter.post('/', (req, res) => {
  ...
  const biomassRevenue = totalHarvestKg * RATES.biofertiliserInrPerKg;
  const creditRevenue = (totalCo2 / 1000) * RATES.creditInrPerTonne;
  ...
  res.json({
    ...
    economics: {
      ...
      biomassRevenueInr: Math.round(biomassRevenue),
      creditRevenueInr: Math.round(creditRevenue),
      netInr: Math.round(biomassRevenue + creditRevenue - totalCost),
      assumptions: RATES,
      note:
        'Harvesting and drying dominate real cost and swing by an order of magnitude ' +
        'with the method chosen. Treat this as a shape, not a quote.',
    },
  });
});
