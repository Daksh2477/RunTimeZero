...
export function ReportLookup() {
  ...
  }}><label htmlFor="report-reference">Have a report reference?</label><div className="input-action"><input id="report-reference" value={reference} onChange={(e) => { setReference(e.target.value); setError(''); }} placeholder="Paste the full report reference" required aria-describedby={error ? 'reference-error' : undefined} aria-invalid={!!error} /><button className="button">Find report →</button></div>{error && <p className="err" id="reference-error" role="alert">{error}</p>}</form>;
}
