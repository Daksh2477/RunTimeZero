...
interface SiteFinance {site:{id:string;name:string};asOf:string;costs:{buildInr:number;gatewayInr:number;runningInr:number;totalInr:number};revenue:{credits:{retirements:number;kg:number;inr:number;unpricedRetirements:number};produce:{orders:number;kg:number;inr:number};totalInr:number};profitInr:number;ponds:{pondId:string;label:string;kit:string;nodes:number;build:{items:{ref:string;part:string;qty:number;unitInr:number;totalInr:number}[];totalInr:number};running:{days:number;energyInr:number;labourInr:number;harvestInr:number;totalInr:number}}[];note:string}
