...
export function PondReadings({ telemetry, pondId }: { telemetry: PondDetail['telemetry']; pondId: string }) {
  ...
  return <section className="panel"><div className="section-heading"><h2>Latest water readings</h2><span className={`live-indicator ${live.connection}`}><span aria-hidden="true">●</span> {state}</span></div><p className="sub">{incoming?.source==='sim'?'Simulation replay time':'Recorded'}: {readingTime(last?.observedAt)}{incoming?.receivedAt&&` · received ${Math.max(0,Math.floor((live.now-incoming.receivedAt)/1000))} s ago`}</p>
