...
interface Eligible {pondId:string;label:string;siteId:string;readings:{firstAt:string|null;lastAt:string|null};checkedJustNow:boolean;credits:{checks:number;flaggedChecks:number;periodStart:string|null;periodEnd:string|null;creditableKg:number;suggestedInrPerTonne:number;estInr:number;disposition:string|null};canApprove:boolean;blockers:string[];harvests:{harvestId:string;harvestedAt:string;kg:number;grade:string;suggestedInrPerKg:number;estInr:number}[]}
...
export function EligibleToSell({pondId}:{pondId:string}) {
 ...
 if(!ready||!role||r.missing)return null;
 if(r.error)return <section className="panel"><h2>Eligible to sell</h2><p role="alert">{r.error}</p><button className="button secondary" onClick={()=>setRevision(n=>n+1)}>Retry eligibility</button></section>;
 ...
 async function approve(){if(busy)return;setBusy(true);setError('');try{setDone(await apiData<Approved>(`/market/eligible/pond/${encodeURIComponent(pondId)}/approve`,{method:'POST',body:JSON.stringify({issueCredits,disposition,dispositionEvidenceRef:reference})}));setRevision(n=>n+1);}catch(e){setError(e instanceof Error?e.message:'Could not list.');}finally{setBusy(false);}}
 return <section className="panel"><p className="eyebrow">MARKETPLACE</p><h2>Eligible to sell</h2>
  ...
  {d&&<><p className="sub">{d.readings.firstAt&&d.readings.lastAt?`Readings from ${dateLabel(d.readings.firstAt)} to ${dateLabel(d.readings.lastAt)}`:'No stored readings yet.'}{d.checkedJustNow?' · carbon check run just now':''}</p>
   <div className="summary-grid"><div className="summary-card"><span>Evidence-supported capture</span><strong className="num">{mass(d.credits.creditableKg)}</strong><small>{d.credits.checks} checks{d.credits.flaggedChecks?` · ${d.credits.flaggedChecks} flagged`:''} · estimated {rupees(d.credits.estInr)} at {rupees(d.credits.suggestedInrPerTonne)}/tonne</small></div><div className="summary-card"><span>Unlisted harvests</span><strong className="num">{d.harvests.length}</strong><small>Estimated {rupees(harvestInr)} at suggested prices</small></div></div>
   ...
   {canApprove&&!done&&d.credits.creditableKg>0&&<div className="market-form"><label>What to list<select value={issueCredits?'credits':'harvests'} onChange={e=>setIssueCredits(e.target.value==='credits')}><option value="harvests">Unsold harvests</option><option value="credits">Removal credits with durable-storage evidence</option></select></label>{issueCredits&&<><label>Durable use<select value={disposition} onChange={e=>setDisposition(e.target.value)}><option value="biochar">Biochar</option><option value="buried">Buried</option><option value="bioplastic">Bioplastic</option></select></label><label>Storage or offtake evidence reference<input required value={reference} onChange={e=>setReference(e.target.value)}/></label><p>Only this pond’s reviewed checks will be issued. Harvests are not listed in this step: the same biomass cannot both be stored durably and sold as feed.</p></>}</div>}
   {d.blockers?.map(b=><p key={b} className="helper">{b}</p>)}
   {done?<div role="status" className="inline-notice"><p>{done.batch?`Batch issued: ${mass(done.batch.creditableCo2Kg)}. ${done.batch.note}`:done.creditNote}</p><p>{done.listedHarvests.length} {done.listedHarvests.length===1?'harvest':'harvests'} listed.</p>{done.batch&&<Link href={`/verify/batch/${done.batch.id}`}>Inspect batch →</Link>} <Link href="/console/market">See it on the market →</Link></div>
    :canApprove&&<button className="button" disabled={busy||nothing||!d.canApprove||(issueCredits?(!reference.trim()||d.credits.creditableKg<=0):d.harvests.length===0)} onClick={approve}>{busy?'Listing…':issueCredits?'Approve and issue these credits':'Approve and list these harvests'}</button>}
