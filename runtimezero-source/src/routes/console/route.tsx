...
import { Leaf, LineChart, FlaskConical, Users, LogOut, Lock } from "lucide-react";
import { ROLE_META } from "@/lib/mockData";
import { canAccess, clearSessionRole, useSessionRole } from "@/lib/session";
...
const NAV = [
  ...
  { to: "/console/investor", label: "Market", icon: LineChart },
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 border-b border-border bg-panel text-panel-foreground">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-6 gap-y-3 px-4 py-3 sm:px-6">
          ...
          {role && (
            <span className="rounded-full bg-panel-foreground/10 px-3 py-1 text-xs font-semibold">
              {role === "admin" ? "Admin" : ROLE_META[role].label}
            </span>
          )}
          <Link
            to="/"
